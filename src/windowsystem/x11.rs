...
impl X11Interface
{
		...
		-> Result<WindowClassHint, GetWindowPropertyError>
	{
		unsafe
		{
			...
			let status = xlib::XGetClassHint(self.display, window, class_hint);
		...
		-> Result<Option<*mut c_uchar>, GetWindowPropertyError>
	{
		...
		let property_atom = xlib::XInternAtom(self.display, property.as_ptr() as *const i8, 0);
	...
	fn send_keysym_sequence(&self, sequence: &[c_uint], pressed: bool, delay: Duration)
	{
		unsafe
		{
			...
			{
				let mut keycode = xlib::XKeysymToKeycode(self.display, (*symbol) as u64);
				...
				if keycode == 0
				{
					...
					xlib::XChangeKeyboardMapping(self.display, keycode as i32, 1, &mut symbol, 1);
					xlib::XSync(self.display, 0);
				}

				xtest::XTestFakeKeyEvent(self.display, keycode as u32, pressed as i32, xlib::CurrentTime);
			...
			{
				let mut symbol = 0;
				xlib::XChangeKeyboardMapping(self.display, temporary_keycode as i32, 1, &mut symbol, 1);
				xlib::XFlush(self.display);
			}
		}
	}
